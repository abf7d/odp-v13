...
import * as CONST from '../../../../services/constants/chart-constants';
...
export class LegendComponent implements OnInit, OnChanges {
  ...
  @Output() variantSelected = new EventEmitter<DisplayLineage>();
  @Output() subvariantSelected = new EventEmitter<Subvariant>();
  @Output() setSubLegend = new EventEmitter<boolean>();
  public visibleMap = new Map<string | null, boolean>();
  public selectedVariant!: DisplayLineage;
  public selectedSubvariant!: Subvariant;
  ...
  public ngOnChanges(changes: SimpleChanges) {
    if (changes?.variants?.currentValue) {
      this.filteredLineages = changes.variants.currentValue.filter(
        x => x.viralLineage !== CONST.whatsNew.viralLineage && x.viralLineage !== CONST.allVariants.viralLineage
      );
    }
    if (changes?.dataPoints?.currentValue) {
      ...
      changes?.dataPoints?.currentValue.forEach(d => this.visibleMap.set(d.viralLineage, true));
      this.filteredLineages.sort((a, b) => {
